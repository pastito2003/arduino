...
#define DEBUG        //Definir esta vaiable para imprimir info de debug en el serial
#define CICLO 120    //Definir el ciclo en segundos
...
#ifdef DEBUG
char buffer[80];            //Variable para imprimir, solo la necesitamos si DEBUG esta definido
#endif
int pote1 = 0;
int pote2 = 0;
unsigned long intervalo;
unsigned long milisPrevio1 = 0;
unsigned long milisPrevio2 = 0;
bool TMOUT1 = false;
bool TMOUT2 = false;
unsigned long segs1 = 0, segs2 = 0;
...
void setup() {
  ...
  intervalo = MAXTIME * 60 * 60 * 1000;
  //intervalo = MAXTIME * 2 * 1000; //Definir un ciclo corto para debuggear
  
  wdt_disable();       //Disable para que nos de tiempo de inicializar todo antes de que se resetee
  wdt_enable(WDTO_8S); // Habilitar al ultimo... 4S de timeout
}


void loop() {
  int segon1, segon2, segoff1, segoff2;
  int switch1, switch2;
...
#ifdef DEBUG  
  sprintf (buffer, "Uptime %lu seconds", millis()/1000);
  Serial.println (buffer);
  Serial.println ();
  sprintf (buffer, "%ld de %ld milisegundos para timeout en 1", milisAhora - milisPrevio1, intervalo);
  Serial.println (buffer);
  sprintf (buffer, "%ld de %ld milisegundos para timeout en 2", milisAhora - milisPrevio2, intervalo);
  Serial.println (buffer);
#endif
  ...
  if (switch1||switch2) { //Solo si alguno de los dos switches está encendido, procesamos
	...
	Serial.println (buffer);
      ...
      milisPrevio1 = milisAhora;       //Aqui, si se puso el switch en off, reseteamo milsPrevio1 así si se enciende vuelve a haber un ciclo de TMOUT1 completo!
      digitalWrite(SALIDA1,LOW);
      TMOUT1 = false;                  //Reset de TMOUT1
      segs1 = 0;
    }
    ...
    if (switch2) {
      if (!TMOUT2) {
        ...
        segon2 = (pote2 * CICLO) / 100;  //Determinar cuantos segundos tiene que estar encendido dentro del ciclo para pote2
        segoff2 = CICLO - segon2;        //Determinar cuantos segundos tiene que estar apagado para pote 2
#ifdef DEBUG
